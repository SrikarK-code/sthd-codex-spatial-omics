...
def merge(save_path, refile):
    ...
    sthdata = train.load_data(allregion_path)
    sthdata, ref_df = train.sthdata_match_refgene(sthdata, refile)
    cell_type_names = ref_df.columns.tolist()
    ...
    sthdata_with_pdata = train.predict(sthdata, pdata_reorder.values, cell_type_names)
    train.save_prediction_pdata(sthdata_with_pdata, file_path=allregion_path)
...
def prepare_constants(sthd_data, K=10):
    ...
    # 1. Build the physical Delaunay graph
    sq.gr.spatial_neighbors(
    ...
    # ---------------------------------------------------------
    # NEW: Anisotropic Feature-Weighting (Biological Boundaries)
    # ---------------------------------------------------------
    rows, cols = A_csr.nonzero()
    
    # Calculate the squared difference in protein space for every connected pair of cells
    # We divide by Var so highly-variable noisy markers don't artificially destroy edges
    diffs = N[rows] - N[cols]
    sq_dists = np.sum((diffs ** 2) / Var, axis=1)
    
    # Convert distance to a similarity weight (0.0 to 1.0) using an exponential kernel
    # We use the median distance to self-tune the sensitivity of the decay
    median_dist = np.median(sq_dists)
    if median_dist == 0: 
        median_dist = 1.0 # Fallback to prevent divide-by-zero
        
    similarities = np.exp(-sq_dists / median_dist)
    
    # Overwrite the binary Delaunay edges with the continuous biological weights
...
def train(n_iter, step_size, beta, constants, weights, early_stop=False, beta1=0.9, beta2=0.999, epsilon=1e-8):
    ...
    for i in range(n_iter):
        ...
        update_V_adam(V, m_V, v_V, beta1, beta2, i + 1, step_size * 0.55, epsilon, K, Z, ll_wat_V)
